...
// DP Grid5 [ first row to Last row ],  min sum
// variable start and end

class Solution {

      int recur2(int i, int j, vector<vector<int>>& grid){
        ...
        int leftDown = recur2(i+1, j-1, grid);
        int down = recur2(i+1, j, grid);
        int rightDown = recur2(i+1, j+1, grid);

        return grid[i][j] + min(down, min(leftDown, rightDown));
      }

      int recur(int i, int j, vector<vector<int>>& grid){

        // Base cases
        if(j<0 || j>=grid.size()){       // Invalid
          return INT_MAX;
        }
        if(i==0){                        // Destination
          return grid[i][j];
        ...
        int leftUp = recur(i-1, j-1, grid);
        int up = recur(i-1, j, grid);
        int rightUp = recur(i-1, j+1, grid);

        return grid[i][j] + min({up, leftUp, rightUp});
      }
      ...
      int recurMemo(int i, int j, vector<vector<int>>& grid, vector<vector<int>>& dp){
        ...
        int leftUp = recurMemo(i-1, j-1, grid, dp);
        int up = recurMemo(i-1, j, grid, dp);
        int rightUp = recurMemo(i-1, j+1, grid, dp);

        return dp[i][j] = grid[i][j] + min({up, leftUp, rightUp});
      }
      ...
      int tabulation(vector<vector<int>>& grid){
        ...
        for(int i=1; i<n; i++){
          for(int j=0; j<n; j++){
            ...
            dp[i][j] = grid[i][j] + min({up, leftUp, rightUp});
      ...
      int spaceOpt(vector<vector<int>>& grid) {
        ...
        for(int i=1; i<n; i++){
          ...
          for(int j=0; j<n; j++){
            ...
            int up = prev[j];
            int leftUp = (j > 0) ? prev[j - 1] : INT_MAX;
            int rightUp = (j < n - 1) ? prev[j + 1] : INT_MAX;

            curr[j] = grid[i][j] + min({up, leftUp, rightUp});
...
public:
    int minFallingPathSum(vector<vector<int>>& matrix) {
        ...
        // Recursion (starting with last row) more ideal to start with destination
        // int ans = INT_MAX;
        // for(int i=0; i<n; i++){
        //   ans = min(ans, recur(n-1, i, matrix));
        // }
        // return ans;

        // Recursion 2    (starting with first row)
        // int ans = INT_MAX;
        // for(int i=0; i<n; i++){
        //   ans = min(ans, recur2(0, i, matrix));
        // }
        // return ans;
        ...
        //   ans = min(ans, recurMemo(n-1, i, matrix, dp));
        // }
        // return ans;
